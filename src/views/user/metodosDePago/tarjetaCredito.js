...
function TarjetaCreditoModal({ closeModalTarjetaCredito, descriptionOrder, dataOrderAddress, discountCoupon, total, cupon, ipAddress, idAddress, setModalPurchaseSuccess, setOk, setModalProcesoPago, setModalProcesoPagoClose, updateAddress }) {
    ...
    const [showTooltipCuotes, setShowTooltipCuotes] = useState(false);
    ...
    /* manejo de cuotas */
    const handleCuotesChangue = (event) => {
        const value = event.target.value;
        setCardCuotes(value);
    }

    const handleCuotesBlur = () => {
        const isValid = /^\d{1,}$/.test(cardCuotes); //Validar que sea un numero entero
        setShowTooltipCuotes(!isValid);
    }
    ...
    const handleSubmitOrderPaymentCard = () => {
        ...
        if (token) {
            ...
            if (showTooltipMonth || showTooltipAnio || showTooltipCvc || showTooltipCuotes) {
                Swal.fire({
           ...
           the discount applied will be used instead of the original total value. */
                ...
                const dataOrder = {
                    ...
                    tipo_pago: 'Tarjeta crédito' //tipo de pago registrado
                }
                                    ...
                                    <InputGroup style={{ width: '40%' }}>
                                        <Input
                                            ...
                                            style={{
                                                borderRadius: "50px",
                                                width: '10.3%',
                                                borderColor: showTooltipCuotes ? 'red' : '',
