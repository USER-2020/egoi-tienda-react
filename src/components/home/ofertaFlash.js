...
import { getOfferFlash, getOfferFlashById } from '../../services/ofertas';


const OfertaFlash = () => {

    const [products, setProducts] = useState([]);
    const [idFlash, setIdFlash] = useState('');
    ...
    const getOffertaFlash = () => {
        getOfferFlash()
            .then((res) => {
                console.log(res.data);
                setIdFlash(res.data.id);
                setEndDate(res.data.end_date);
                getOfferFlashID(res.data.id);
            }).catch((err) => console.log(err));
    }

    const getOfferFlashID = (idOffer) => {
        getOfferFlashById(idOffer)
            .then((res) => {
                console.log(res.data);
                ...
                <div className='container'>
                    <div className='containerRecents'>
                        <div className='spanRecent' style={{
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-battery-charging" viewBox="0 0 16 16">
                            ...
                            <p style={{ backgroundColor: '#FC5241', padding: '5px', borderRadius: '5px', color:'white', marginBottom:'0', alignSelf:'center', fontSize:'16px' }}>
                                {timeLeft}
                            </p>
                            {/* <Link to='/addRecently'>
