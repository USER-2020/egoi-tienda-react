...
const DetailCartNoToken = ({ productsInCart, getAllProductsByCartNotoken, setIsLoggedInPartner }) => {
    ...
    useEffect(() => {
        console.log("Productos del carrito en el detail cart sin token ", productsInCart);
        costoDeENvio();
                                    ...
                                    <div className="caracteristicasDetalleDelProductoResponsive">
                                        ...
                                        <div className="info">
                                            ...
                                            <div className="productInfo">
                                                ...
                                                <div className="cant">
                                                    <p>Cantidad:</p>
                                                    {/* <input type="number" value={products.min_qty} disabled /> */}
                                                    {products.min_qty >= 2 && (
                                ...
                                {productsInCart && productsInCart.map((products, index) => (
                                    <Card key={index}>
                                        ...
                                        <div className="caracteristicaPrecio">
                                            {products.discount === 0 && products.discount_tag_valor === 0 && (
                                                <>
                                                    ${products.unit_price.toLocaleString('es')}
                                                </>
                                            )}
                                            {products.discount_tag_valor > 0 && (
                                                <>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                                        <p style={{ marginBottom: '0px', color: '#A2A1A7' }}><s>$ {(products.unit_price.toLocaleString('es'))}</s></p>
                                                        <p style={{ marginBottom: '0px' }}>$ {(products.discount_tag).toLocaleString('es')}</p>
                                                    </div>
                                                </>
                                            )}
                                            {products.discount > 0 && (
                                                <>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                                        <p style={{ marginBottom: '0px', color: '#A2A1A7' }}><s>$ {(products.unit_price.toLocaleString('es'))}</s></p>
                                                        <p style={{ marginBottom: '0px' }}>$ {(products.unit_price - products.discount).toLocaleString('es')}</p>
                                                    </div>
                                                </>
                                            )}
                                        </div>
                                        <div className="caracteristicaCantidad">
                                            {products.min_qty >= 2 && (
                                                ...
                                                <button className='btnIzq' onClick={() => { setQuantity(products.min_qty); handleDecrement(products.min_qty, products.id) }}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-dash" viewBox="0 0 16 16">
                                                        <path d="M4 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 4 8z" />
                                                    </svg>
                                                </button>
                                            ...
                                            <button className='btnDer' onClick={() => { setQuantity(products.min_qty); handleIncrement(products.min_qty, products.id) }}>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-lg" viewBox="0 0 16 16">
                                                    <path fill-rule="evenodd" d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2Z" />
                                                </svg>
                                            </button>
                                        ...
                                        <div className="caracteristicaPrecioTotal">
                                            {products.discount === 0 && products.discount_tag_valor === 0 && (
                                                <>
                                                    ${((products.unit_price) * products.min_qty).toLocaleString('es')}
                                                </>
                                            )}
                                            {products.discount > 0 && (
                                                <>
                                                    $ {((products.unit_price - products.discount) * products.min_qty).toLocaleString('es')}
                                                </>
                                            )}
                                            {products.discount_tag_valor > 0 && (
                                                <>
                                                    {(products.discount_tag_valor * products.min_qty).toLocaleString('es')}
                                                </>
                                            )}
                                        </div>
...
export default DetailCartNoToken;
