  ...
  const handleEnterPress = (event) => {
    if (event.key === "Enter") {
      ...
      if (categoriesSearch.length === 0) {
        ...
        history.push(
          `/categories/${categoriesSearch[0].name}/${categoriesSearch[0].name}/${categoriesSearch[0].id}`
        );
      }
      setShowResults(false);
    }
      ...
      <nav class="navbar bg-body-tertiary fixed-top">
        <div class="container-fluid">
          ...
          <a class="navbar-brand" href="/">
            <img src={logo} alt="logo" className={styles.logo} width={""} />
          </a>
                        ...
                        {products.map((result, index) => (
                          <li key={index} className="searchResultItem">
                            <a
                              href=""
                              onClick={() =>
                                handleClickResultProduct(result.id, result.slug)
                              }
                            >
                              {result.name}
                            </a>
          ...
          <a
            ...
            style={{
              ...
              pointerEvents: shouldBtnCart ? "none" : "auto",
              display: shouldBtnCart ? "none" : "visible",
            }}
          >
            <svg
            ...
            <Offcanvas.Body>
              {/* <div
                ...
                class={`offcanvas offcanvas-end ${
                  isOffcanvasVisible ? "show" : "collapse"
                }`}
                tabindex="-1"
                id="offcanvasRight"
              ...
              {/* <button
                  ...
                  {currenUser && detailInfoProfile && (
                    <h5 class="offcanvas-title" id="offcanvasNavbarLabel">
                      ¡Hola,{" "}
                      {detailInfoProfile.f_name +
                        " " +
                        detailInfoProfile.l_name}
                      !
                    </h5>
                  )}
                </div> */}
              <div className={`offcanvas-body`}>
                <ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
                          ...
                          <a
                            ...
                            href={
                              category.childes.length === 0
                                ? `/categories/${category.name}/${category.name}/${category.id}`
                                : null
                            }
                          >
                            <span
                          ...
                          <ul>
                            {category.childes.map((subcategory, index) => (
                              <li key={index}>
                                ...
                                <a
                                  className={`dropdown-item subcategory-item  ${category.name !==
                                    selectedCategory && "visually-hidden"}`}
                                  href={`/categories/${category.name}/${subcategory.name}/${subcategory.id}`}
                                >
                                  {subcategory.name}
                                </a>
                                {subcategory.childes.map((brand, index) => (
                                  <li key={index}>
                                    <a
                                      className={`dropdown-item  px-5 ${category.name !==
                                        selectedCategory && "visually-hidden"}`}
                                      href={`/categories/${category.name}/${brand.name}/${brand.id}`}
                                    >
                                      {brand.name}
                                    </a>
                                  </li>
                    ...
                    <ul class="dropdown-menu dropdown-menu-extended">
                      {brands.map((brand, index) => (
                        <li key={index}>
                          <a
                            className="dropdown-item"
                            href={`/brand/${brand.name}/${brand.id}`}
                          >
                            <strong>
                              {brand.name}({brand.brand_products_count})
                            </strong>
                          </a>
                        </li>
                      ))}
                  ...
                  <li class="nav-item dropdown">
                    <a
                      class="nav-link dropdown-toggle w-full"
                      href="#"
                      role="button"
                      data-bs-toggle="dropdown"
                      aria-expanded="false"
